...
impl MultiTokenManager {
    ...
    /// 并集查询指定分组内所有可用凭据的上游可用模型（供公开 `/v1/models` 使用）
    ///
    /// - 枚举分组内「未禁用」凭据（`group=None` 表示全部），逐个串行查询上游
    ///   `ListAvailableModels`，避免并发压上游。
    /// - 任一凭据成功即并入结果，按 `model_id` 去重保序（订阅等级不同 → 模型集合不同）。
    /// - 无可用凭据、或全部凭据查询失败时返回 `Err`，由调用方决定回退策略。
    pub async fn get_available_models_union(
        &self,
        group: Option<&str>,
    ) -> anyhow::Result<Vec<UpstreamModel>> {
        // 先在持锁下拍下候选凭据 ID（未禁用 + 分组匹配），随即释放锁再 await
        let ids: Vec<u64> = {
            let entries = self.entries.lock();
            entries
                .iter()
        ...
        if ids.is_empty() {
            bail!("分组内无可用凭据，无法查询可用模型");
        }

        let mut merged: Vec<UpstreamModel> = Vec::new();
        ...
        for id in ids {
            match self.get_available_models_for(id).await {
                ...
                Err(e) => {
                    tracing::warn!("凭据 #{} 查询可用模型失败（跳过）: {}", id, e);
                    last_error = Some(e);
                }
            }
        }
        ...
        if any_success {
            ...
            Err(last_error
                .unwrap_or_else(|| anyhow::anyhow!("所有凭据查询可用模型均失败")))
        }
    }
