...
pub struct KiroProvider {
    ...
    /// 公开 `/v1/models` 的可用模型缓存（key = 分组，None = 全部凭据）。
    ///
    /// 并集查询会对分组内每张凭据各打一次上游 `ListAvailableModels`，请求量随凭据数
    /// 线性增长；用 TTL 缓存兜住高频拉取。value = （写入时刻, 模型列表）。
    models_cache: Mutex<HashMap<Option<String>, (Instant, Vec<UpstreamModel>)>>,
}

/// 可用模型缓存 TTL：10 分钟。
///
/// 兼顾「兜住高频 `/v1/models`」与「新凭据 / 订阅变更能较快反映」。
...
impl KiroProvider {
    ...
    /// 查询指定分组的上游可用模型（带 TTL 缓存），供公开 `/v1/models` 使用。
    ///
    /// - 命中且未过期 → 直接返回缓存。
    /// - 未命中 / 已过期 → 并集查询分组内所有可用凭据的上游 `ListAvailableModels`，
    ///   成功则写缓存返回。
    /// - 查询失败但存在旧缓存（即便已过期）→ 回退旧缓存，避免上游抖动时抖动对外结果。
    /// - 查询失败且无任何缓存 → 返回 `Err`，由调用方回退到本地硬编码列表。
    pub async fn list_available_models(
        &self,
        group: Option<&str>,
    ) -> anyhow::Result<Vec<UpstreamModel>> {
        let key = group.map(|s| s.to_string());

        // 命中未过期缓存：持锁下拷贝返回（clone 为同步操作，不跨 await 持锁）
        {
            let cache = self.models_cache.lock();
            if let Some((written_at, models)) = cache.get(&key)
                && written_at.elapsed() < MODELS_CACHE_TTL
            {
                return Ok(models.clone());
            }
        }

        match self.token_manager.get_available_models_union(group).await {
            Ok(models) => {
                self.models_cache
                    .lock()
            ...
            Err(e) => {
                // 上游全失败：回退到旧缓存（若有），否则上抛让调用方回退硬编码列表
                let stale = self.models_cache.lock().get(&key).map(|(_, m)| m.clone());
                match stale {
                    Some(models) => {
                        tracing::warn!(
                            "并集查询可用模型失败，回退到过期缓存（{} 个模型）: {}",
                            models.len(),
                            e
                        );
                        Ok(models)
