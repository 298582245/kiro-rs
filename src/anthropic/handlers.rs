...
/// 可用模型缺省输出上限（上游 `ListAvailableModels` 只给输入窗口，不给输出上限）
const DEFAULT_MODEL_MAX_TOKENS: i32 = 64000;

/// 动态模型的占位创建时间（上游不下发 created，用固定占位值对齐对外结构）
const DYNAMIC_MODEL_CREATED: i64 = 1782000000;

/// 将上游 `ListAvailableModels` 返回的模型透传映射为对外 `Model`
///
/// - `id`：上游 modelId 原样透传（点号形式，如 `claude-opus-4.6`；`map_model` 用
///   contains 匹配，不影响 `/v1/messages` 路由）。
/// - `owned_by`：由 id 前缀推断（`gpt*` → openai，否则 anthropic）。
/// - `display_name`：优先 modelName，缺省回退 modelId。
/// - `max_tokens`：固定为输出上限缺省值（上游只给输入窗口）。
fn upstream_to_models(models: Vec<UpstreamModel>) -> Vec<Model> {
    models
        .into_iter()
        .map(|m| {
            let owned_by = if m.model_id.to_ascii_lowercase().starts_with("gpt") {
                "openai"
            } else {
                "anthropic"
            };
            let display_name = m.model_name.clone().unwrap_or_else(|| m.model_id.clone());
            Model {
                id: m.model_id,
                object: "model".to_string(),
                created: DYNAMIC_MODEL_CREATED,
...
/// GET /v1/models
///
/// 返回可用的模型列表。优先按当前 Key 所属分组并集查询上游真实可用模型（带 TTL 缓存）；
/// provider 未配置或上游查询失败时回退到本地硬编码列表，保证端点始终可用。
pub async fn get_models(
    ...
    let models = match &state.kiro_provider {
        Some(provider) => {
            match provider.list_available_models(key_ctx.group.as_deref()).await {
                Ok(upstream) if !upstream.is_empty() => upstream_to_models(upstream),
                Ok(_) => {
                    tracing::warn!("上游可用模型列表为空，回退到本地硬编码列表");
                    available_models()
                }
                Err(e) => {
                    tracing::warn!("查询上游可用模型失败，回退到本地硬编码列表: {}", e);
                    available_models()
                }
            }
        }
        None => {
            tracing::warn!("KiroProvider 未配置，/v1/models 回退到本地硬编码列表");
            available_models()
        }
    };
...
mod tests {
    ...
    #[test]
    fn upstream_to_models_passes_through_dotted_id_and_infers_owner() {
        let upstream = vec![
            UpstreamModel {
        ...
        let models = upstream_to_models(upstream);
        assert_eq!(models.len(), 2);

        // 点号 ID 原样透传；modelName 作为 display_name
        ...
        assert_eq!(models[0].max_tokens, DEFAULT_MODEL_MAX_TOKENS);

        // gpt* → openai；缺省 modelName 时 display_name 回退到 modelId
    ...
    #[test]
    fn upstream_to_models_handles_empty() {
        assert!(upstream_to_models(Vec::new()).is_empty());
    }
}
